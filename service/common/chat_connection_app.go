...
func PostOttAccount(ottDomain, ottVersion string, chatApp model.ChatApp, connectionApp model.ChatConnectionApp) error {
	...
	if chatApp.InfoApp.Facebook.Status == "active" && len(connectionApp.OaInfo.Facebook) > 0 {
	...
	} else if chatApp.InfoApp.Zalo.Status == "active" && len(connectionApp.OaInfo.Zalo) > 0 {
