...
func (s *Conversation) InsertNoteInConversation(ctx context.Context, authUser *model.AuthUser, data *model.ConversationNoteRequest) (id string, err error) {
	...
	if conversationExist.NotesList == nil {
		conversationExist.NotesList = &[]model.NotesList{}
	}
	if len(*conversationExist.NotesList) > CONVERSATION_NOTES_LIST_LIMIT {
		err = errors.New("out of limit of notes list in conversation")
		log.Error(err.Error())
		return
	}
	id = uuid.NewString()
	*(conversationExist.NotesList) = append(*(conversationExist.NotesList), model.NotesList{
		Id:        id,
	...
	if len(*allocateUser) > 0 {
		...
		s.publishConversationEventToManagerAndAdmin(authUser, manageQueueUser, variables.EVENT_CHAT["conversation_add_note"], conversationConverted)
	} else {
		s.publishConversationEventToManagerAndAdmin(authUser, nil, variables.EVENT_CHAT["conversation_add_note"], conversationConverted)
	}
	return
}

func (s *Conversation) UpdateNoteInConversationById(ctx context.Context, authUser *model.AuthUser, id string, data model.ConversationNoteRequest) (err error) {
	conversationExist, err := repository.ConversationESRepo.GetConversationById(ctx, authUser.TenantId, ES_INDEX_CONVERSATION, data.AppId, data.ConversationId)
	if err != nil {
		log.Error(err)
		return
	...
	if conversationExist.NotesList == nil || len(*conversationExist.NotesList) < 1 {
		err = errors.New("note " + id + " not found")
		log.Error(err.Error())
		return
	}
	found := false
	for i := 0; i < len(*conversationExist.NotesList); i++ {
		if (*conversationExist.NotesList)[i].Id == id {
			(*conversationExist.NotesList)[i].Content = data.Content
			(*conversationExist.NotesList)[i].UpdatedAt = time.Now()
			found = true
			break
		}
	}
	...
	if len(*allocateUser) > 0 {
		...
		s.publishConversationEventToManagerAndAdmin(authUser, manageQueueUser, variables.EVENT_CHAT["conversation_update_note"], conversationConverted)
	} else {
		s.publishConversationEventToManagerAndAdmin(authUser, nil, variables.EVENT_CHAT["conversation_update_note"], conversationConverted)
	}
	return
}

func (s *Conversation) DeleteNoteInConversationById(ctx context.Context, authUser *model.AuthUser, id string, data model.ConversationNoteRequest) (err error) {
	conversationExist, err := repository.ConversationESRepo.GetConversationById(ctx, authUser.TenantId, ES_INDEX_CONVERSATION, data.AppId, data.ConversationId)
	if err != nil {
		log.Error(err)
		return
	...
	for i := 0; i < len(*conversationExist.NotesList); i++ {
		if (*conversationExist.NotesList)[i].Id == id {
			*conversationExist.NotesList = append((*conversationExist.NotesList)[:i], (*conversationExist.NotesList)[i+1:]...)
			found = true
			break
		}
	}
	...
	if len(*allocateUser) > 0 {
		...
		s.publishConversationEventToManagerAndAdmin(authUser, manageQueueUser, variables.EVENT_CHAT["conversation_delete_note"], conversationConverted)
	} else {
		s.publishConversationEventToManagerAndAdmin(authUser, nil, variables.EVENT_CHAT["conversation_delete_note"], conversationConverted)
	}
	return
}

func (s *Conversation) GetConversationNotesList(ctx context.Context, authUser *model.AuthUser, filter model.ConversationNotesListFilter, limit, offset int) (total int, result []*model.NotesList, err error) {
	filter.TenantId = authUser.TenantId
	total, result, err = repository.ConversationESRepo.GetNotesList(ctx, authUser.TenantId, ES_INDEX_CONVERSATION, filter, limit, offset)
	if err != nil {
		log.Error(err)
		return
	}
