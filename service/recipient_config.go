...
type (
	IRecipientConfig interface {
		InsertRecipientConfig(ctx context.Context, authUser *model.AuthUser, data model.RecipientConfigRequest) error
...
func (s *RecipientConfig) InsertRecipientConfig(ctx context.Context, authUser *model.AuthUser, data model.RecipientConfigRequest) error {
	...
	for _, val := range data.Recipient {
		...
		filter := model.RecipientConfigFilter{
			...
			Recipient:     []string{val},
		}
		total, _, err := repository.RecipientConfigRepo.GetRecipientConfigs(ctx, dbCon, filter, 1, 0)
		if err != nil {
			log.Error(err)
			return err
		}
		if total > 0 {
			return fmt.Errorf("recipient %s with provider %s already exist", val, data.Provider)
		}
		recipientConfigTmp := model.RecipientConfig{
			Base:          model.InitBase(),
			Recipient:     val,
	...
	if err := repository.RecipientConfigRepo.BulkInsertRecipientConfig(ctx, dbCon, recipientConfig); err != nil {
		log.Error(err)
		return err
	}
